  ...
  if st.session_state['cap2']=='2':
    with st.form('capitolul 2 si 3'):
        ...
        if submitted:
                ...
                try:
                    #schimba_M_3_1(data1['numarore'].loc[(data1['specializare']==st.session_state['M_1_6'])&(data1['nume_disciplina']==st.session_state['M_2_1'])&(data1['curs']=='CURS      ')].values[0]+data1['numarore'].loc[(data1['nume_disciplina']==add_selectbox_D)&(data1['curs']!='CURS      ')].values[0])
                    #schimba_M_3_4(14*(data1['numarore'].loc[(data1['specializare']==st.session_state['M_1_6'])&(data1['nume_disciplina']==st.session_state['M_2_1'])&(data1['curs']=='CURS      ')].values[0]+data1['numarore'].loc[(data1['nume_disciplina']==add_selectbox_D)&(data1['curs']!='CURS      ')].values[0]))
                    ...
                    try: 
                        ...
                        try:
                            ...
                            pass
                try:
                    #st.write('Numarul de ore de curs:',data1['numarore'].loc[(data1['specializare']==st.session_state['M_1_6'])&(data1['nume_disciplina']==add_selectbox_D)&(data1['curs']=='CURS      ')].values[0])

                    #schimba_M_3_2(data1['numarore'].loc[(data1['specializare']==st.session_state['M_1_6'])&(data1['nume_disciplina']==st.session_state['M_2_1'])&(data1['curs']=='CURS      ')].values[0])
                    #schimba_M_3_5(14*data1['numarore'].loc[(data1['specializare']==st.session_state['M_1_6'])&(data1['nume_disciplina']==st.session_state['M_2_1'])&(data1['curs']=='CURS      ')].values[0])
      ...
      if submitted:
        ...
        try:
            ...
            if os.path.exists(file_path):
                ...
                # Extract fields from st.session_state that match the dataframe columns
                #new_row = {col: fix_encoding(st.session_state.get(col, '')) for col in df.columns}
                ...
                # If the file does not exist, create a new dataframe with columns from st.session_state
                new_row = {key: st.session_state.get(key, '') for key in st.session_state}
                
                # Create a new dataframe with the columns from st.session_state
                df = pd.DataFrame([new_row])
                

            # Step 4: Save the dataframe back to the CSV file
            df.to_csv(file_path, index=False)

            # Print the dataframe to verify
            st.write(df["M_9"])
            
         #st.write('Fisa cu numele '+file_name+' a fost salvata in radacina de pe partitia D')
        except:
            st.write('Fisa cu numele '+file_name+' nu a putut fi generata')
          #try:
            
           #document.write('c:\\'+ file_name)
           #st.write('Fisa cu numele '+file_name+' a fost salvata in radacina de pe partitia C')
          #except:
            #st.write('Fisa cu numele '+file_name+' nu a putut fi salvata pe unitatea locala')
